class PhotoBooth {
    constructor() {
        ...
        try {
            if (typeof FrameHandler === 'undefined') {
                console.error('[PhotoBooth] FrameHandler class not found! Make sure frame-handler.js is loaded.');
                // Create a fallback frame handler
                this.frameHandler = {
                    getAvailableFrames: () => [],
                    addFrameConfigToTemplate: () => {},
                    generateSimpleTemplatePreview: () => '<div class="text-xs text-gray-500">Template</div>',
                    generateTemplateWithPhotos: () => '',
                    getCanvasDimensions: () => [800, 600],
                    drawTemplateOnCanvasAsync: async() => {},
                    getPreviewStyle: () => ''
                };
            } else {
                this.frameHandler = new FrameHandler();
                console.log('[PhotoBooth] FrameHandler initialized successfully');
            }
        } catch (error) {
            console.error('[PhotoBooth] Error initializing FrameHandler:', error);
            // Create a fallback frame handler
            this.frameHandler = {
                getAvailableFrames: () => [],
                addFrameConfigToTemplate: () => {},
    ...
    getTemplateLayouts(photoCount, aspectRatio) {
        ...
        // Template: 3-Photo Vertical dengan multiple frame options
        if (photoCount >= 3) {
            try {
                // Validate frameHandler and method exists
                if (!this.frameHandler) {
                    console.error('[PhotoBooth] frameHandler is not initialized');
                    // Create fallback template
                    templates.push({
                        name: '3-Photo Vertical',
                        layout: 'custom-vertical',
                        description: 'Template dengan 3 foto',
                        frames: [
                ...
                if (typeof this.frameHandler.getAvailableFrames !== 'function') {
                    console.error('[PhotoBooth] frameHandler.getAvailableFrames is not a function. frameHandler:', this.frameHandler);
                    // Create fallback template
                    templates.push({
                        name: '3-Photo Vertical',
                        layout: 'custom-vertical',
                ...
                // Get all available frames for this aspect ratio
                const availableFrames = this.frameHandler.getAvailableFrames(aspectRatio);

                console.log(`[PhotoBooth] Generating templates for ${availableFrames.length} frames`);

                if (availableFrames.length === 0) {
                    console.warn('[PhotoBooth] No frames available, creating fallback template');
                    // Fallback: create at least one template even if no frames
                    ...
                    // Create a template for each available frame
                    availableFrames.forEach((frameConfig, index) => {
                        try {
                            const template = {
                                name: frameConfig.name || `3-Photo Vertical ${index + 1}`,
                                layout: 'custom-vertical',
                                description: `Template dengan ${frameConfig.name}`,
                                frames: [
                                    { width: '100%', height: '33.33%', top: '0%', left: '0%' },
                                    { width: '100%', height: '33.33%', top: '33.33%', left: '0%' },
                                    { width: '100%', height: '33.33%', top: '66.66%', left: '0%' }
                            ...
                            // Add frame PNG configuration using FrameHandler with specific frameId
                            this.frameHandler.addFrameConfigToTemplate(template, aspectRatio, frameConfig.id);

                            console.log(`[PhotoBooth] Created template: ${template.name} with frame: ${template.framePath}`);

                            templates.push(template);
                        } catch (frameError) {
                            console.error(`[PhotoBooth] Error creating template for frame ${frameConfig.id}:`, frameError);
                            // Continue with next frame even if one fails
                        }
                    });
                }
            } catch (error) {
                console.error('[PhotoBooth] Error in getTemplateLayouts:', error);
                // Return at least one fallback template
                templates.push({
                    name: '3-Photo Vertical',
                    layout: 'custom-vertical',
                    description: 'Template dengan 3 foto',
        ...
        console.log(`[PhotoBooth] Returning ${templates.length} templates`);
        return templates;
    }
